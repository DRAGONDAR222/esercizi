...
a = int(input())
b = int(input())
n1 = min(a , b)    # cos├¼ facendo mi assicuro che "a" sia SEMPRE il pi├╣ piccolo tra i numeri
n2 = max(a , b)

risultato:int = n1    # partiamo dall' n1 iniziale per far si che la prima moltiplicazione non risulti 0

while True:
    if n1 != n2:        # aumento finch├¿ non raggiungo il "valore numerico" di b (includendolo di conseguenza)
        n1 += 1
        risultato *= n1    # moltiplico il risultato precedente ad ogni ciclo
        print(risultato)
    else:
        print("il precedente ├¿ il risultato finale")
        break
