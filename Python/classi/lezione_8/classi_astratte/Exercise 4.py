...
Display the state of the university: at each significant step, print out a summary of the universityÔÇÖs current state. This might include:
...
class Professor(Person):
    ...
    def assign_to_course(self,course_assign:"Course") -> None:
            ...
            self.courses.append(course_assign)               # un altro modo sarebbe richiamare direttamente la funzione dell'altra classe (al fine di non entrare in un circolo di recursivit├á bisogna implementare un if statement che verifichi se la modifica sia gi├á avvenuta)
...
# Creazione di un'istanza dell'universit├á
universit├á = University("Universit├á Tecnologica")

# Creazione dei dipartimenti
informatica = Department("Informatica")
letteratura = Department("Letteratura")

# Aggiunta dei dipartimenti all'universit├á
universit├á.add_department(informatica)
universit├á.add_department(letteratura)
...
# Aggiunta degli studenti all'universit├á
universit├á.add_student(studente_alice)
universit├á.add_student(studente_bob)

# Iscrizione degli studenti ai corsi
studente_alice.enroll(strutture_dati)
studente_bob.enroll(letteratura_medievale)

# Visualizzazione dello stato dell'universit├á
print("\nDettagli dell'Universit├á:")
print(universit├á)

print("\nDipartimenti:")
for dipartimento in universit├á.departments:
    print(dipartimento)
