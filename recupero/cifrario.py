...
def caesar_cypher_encrypt(s:str, key:int)-> str | None:
    messaggio:str = ""
    if key > 0:
        for elemento in s:
            if re.fullmatch(r"[a-z]",elemento):
                index:int = alfabeto.index(elemento)
                index += key
                while True:
                    if index >= len(alfabeto):
                        ...
                        break   
                messaggio += alfabeto[index]
            elif re.fullmatch(r"[A-Z]",elemento):
                index:int = alfabeto.index(elemento.lower())
                index += key
                while True:
                    if index >= len(alfabeto):
                        index %= len(alfabeto) 
