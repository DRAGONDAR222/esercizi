from classes.bid_ut import bid_ut
from classes.asta_bid import asta_bid
from classes.OggettoDelPost import Asta
from classes.Utente import UtentePrivato
...
class Bid:
    ...
    _asta_bid_link: asta_bid    # imm, certamente noto alla nascita
    _bid_ut_link: bid_ut        # imm, certamente noto alla nascita
    
    def __init__(self, *, asta: Asta, up: UtentePrivato) -> None:
        self.set_istante(datetime.datetime.now())
        self._add_link_asta_bid(asta)
        self._add_link_bid_ut(up)
        
    def set_istante(self, istante: datetime) -> None:
